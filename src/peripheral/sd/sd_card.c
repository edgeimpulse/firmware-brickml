...
#define SD_CMD0_GO_IDLE_STATE_RETRIES   (2)
#define SD_CMD0_GO_IDLE_STATE_DELAY     (50u)
...
static uint32_t sd_go_idle_state(sd_card_t *pSD) {
    ...
    for (int i = 0; i < SD_CMD0_GO_IDLE_STATE_RETRIES; i++) {
        ...
        sd_release(pSD);
        vTaskDelay(pdMS_TO_TICKS(SD_CMD0_GO_IDLE_STATE_DELAY));
        sd_acquire(pSD);
    }
    return response;
}
...
int sd_init_card(sd_card_t *pSD) {
    ...
    if (!pSD->mutex){
        pSD->mutex = xSemaphoreCreateMutex();
    }
    sd_lock(pSD);
    ...
    if (SD_BLOCK_DEVICE_ERROR_NONE != err) {
        ...
        sd_unlock(pSD);
        if (SD_BLOCK_DEVICE_ERROR_NO_DEVICE == err) {
            pSD->m_Status |= STA_NODISK;    // set sd not present
        }
        return pSD->m_Status;
    }
...
bool sd_init_driver() {
    ...
    if (!initialized) {
        for (size_t i = 0; i < sd_get_num(); ++i) {
            ...
            pSD->spi->mutex = xSemaphoreCreateMutex();
