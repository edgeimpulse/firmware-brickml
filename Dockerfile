...
RUN apt-get update && apt-get -y --fix-missing --no-install-recommends install \
    ...
    unzip rsync wget libpython2.7 curl xmlstarlet git git-lfs python3-pip python-is-python3 srecord && \
    pip3 install cryptography==3.3.2 intelhex click cbor cbor2 && \
